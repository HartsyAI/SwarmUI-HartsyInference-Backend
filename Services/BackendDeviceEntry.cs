...
/// <param name="BackendId">Swarm's backend id, for log lines.</param>
...
/// <param name="FitProfileKey">Device plus every setting that changes a memory verdict (VRAM tier and the
/// text-encoder/VAE/shard placements). Two backends with one key always get the same verdict, so a request is assessed
/// once per key rather than once per backend.</param>
/// <param name="Engine">The backend's engine, which assesses fit against its own device, placement and policy.</param>
public sealed record BackendDeviceEntry(int BackendId, string DeviceKey, string DeviceName, string FitProfileKey,
