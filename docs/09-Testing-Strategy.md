...
| Parity | Custom harness | Same prompt + seed + model on Comfy and SharpInference; diff outputs |

Performance / regression / soak tests live alongside the parity harness.

## Unit tests

Live under `tests/SwarmUI-SharpInference.Tests/` (separate csproj). Don't pull in
SharpInference compute — they should run on any machine in <1s.

What to cover:

- **`SamplerMap`:** every Swarm sampler name maps to an expected SharpInference
...
Mock SharpInference dependencies behind a tiny `IInferenceEngine` interface in
`Generation/InferenceEngineStub.cs` so tests don't need real models.

## Component tests

Live under `tests/SwarmUI-SharpInference.Component.Tests/`. Require:
...
`SHARPINFERENCE_RUN_COMPONENT_TESTS=1` env var.
...
3. Generates each case once via Swarm + SharpInference backend (HTTP API).
4. Computes:
...
for "the user couldn't tell the difference." They tighten as SharpInference's numerical
stability improves.
...
- [ ] Configure SharpInference backend, save, restart backend, status reaches IDLE
...
|-------|----------|---------|----------------|
| SDXL 1024×1024 30 steps | RTX 4090 | SharpInference CUDA | within 1.5× of Comfy |
| SDXL 1024×1024 30 steps | RTX 4090 | SharpInference Vulkan | within 2× of Comfy |
| SD1.5 512×512 20 steps | M1 Max | SharpInference CPU | <60s |
| Flux schnell 1024×1024 4 steps | RTX 4090 | SharpInference CUDA | within 1.5× of Comfy |
...
## Bugs found in SharpInference

If a parity test fails because SharpInference has a numerical bug, that's an
**upstream issue**, not an extension bug. We file it against
`/home/kalebbroo/Desktop/Projects/SharpInference/` and either work around it
